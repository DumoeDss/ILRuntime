...
namespace TestCases
{
    public class DelegateTest
    {
        ...
        {
            // DelegateTest44 sets AllowUnboundCLRMethod=false; this method is the
            // ONLY place it is restored to true, so the restore MUST be robust to
            // an exception in the delegate body below. Without the finally, a
            // failure here (e.g. the Neo MethodDelegateAdapter->Action cast gap,
            // tracked as the C1 cluster, which throws at the += below) leaves the
            // flag false for the REST of the suite, cascading a spurious
            // "X is not bound!" NotSupportedException onto every later test that
            // calls a non-autogenerated CLR method -- notably ALL Hotfix* patched-
            // IL tests (the C9 cluster). The finally guarantees the restore
            // regardless; the inline =true is kept before TestEvent4 to preserve
            // the original flag-true timing for that call.
            try
            {
