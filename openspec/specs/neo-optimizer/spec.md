...
*Step-25 partial-ship scope note.* The four requirements above describe the S1 SHIPPED scope only: non-generic methods in the SAME AppDomain as the compile. S2 (generic-instantiation-at-load -- binding the Step-22 `TemplateTable` to live generic-method definitions via `CloneAndPatch` at attach time) and S3 (full `ILType` Cecil-decoupling + cross-AppDomain token-hash re-resolution + `.cctor` seeding) are DEFERRED follow-up scopes; the canonical spec does NOT describe them as met. The S1 loader consumes only the `NeoAssemblyModel.MethodDefs` (non-generic), does NOT consume the `TemplateTable`, and does NOT perform cross-AppDomain hash re-resolution.
