...
*Step-25 / Step-26 partial-ship scope note.* The S1 requirements (non-generic methods, same-AppDomain), the S2 requirements (generic-instantiation-at-load for the no-T-identity-token slice via `CloneAndPatch` from the `.neo` `TemplateTable`), and the Step-26 requirements above (the host-side benchmark self-check + the single-threaded-cooperative contract) are SHIPPED. DEFERRED follow-up scopes (the canonical spec does NOT describe these as met): S3 (full `ILType` Cecil-decoupling + cross-AppDomain token-hash re-resolution + `.cctor` seeding + the T-identity-token / EH-bearing / Cecil-free-AppDomain generic cases); the Neo debugger variable-inspection (`neo-debugger-neo-frame`); and the F-4 reflection-on-Neo field-read family (callvirt-on-CLR-interface / `appdomain.Invoke` instance-reentry / `callvirt.clr GetType` / the Neo `ILTypeInstance` indexer). The S1 loader consumes `NeoAssemblyModel.MethodDefs` (non-generic); the S2 loader additionally consumes the `TemplateTable` (no-T-identity-token generic definitions, skipping T-identity-token / try-catch / unresolvable-VariableTypes records to JIT). Neither performs cross-AppDomain hash re-resolution (S3). The Step-26 benchmark self-check measures interpreter throughput on a fixed workload and asserts measurement correctness; it does NOT assert a Neo-vs-Legacy ratio threshold (the ratio is reported by the separate runner script across two CLI invocations).