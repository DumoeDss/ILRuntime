...
*Step-25 partial-ship scope note.* The S1 requirements (non-generic methods, same-AppDomain) and the S2 requirements above (generic-instantiation-at-load for the no-T-identity-token slice via `CloneAndPatch` from the `.neo` `TemplateTable`) are SHIPPED. S3 (full `ILType` Cecil-decoupling + cross-AppDomain token-hash re-resolution + `.cctor` seeding + the T-identity-token / EH-bearing / Cecil-free-AppDomain generic cases) remains a DEFERRED follow-up scope; the canonical spec does NOT describe S3 as met. The S1 loader consumes `NeoAssemblyModel.MethodDefs` (non-generic); the S2 loader additionally consumes the `TemplateTable` (no-T-identity-token generic definitions, skipping T-identity-token / try-catch / unresolvable-VariableTypes records to JIT). Neither performs cross-AppDomain hash re-resolution (S3).