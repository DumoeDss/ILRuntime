# Capability: neo-debugger

Neo debugger support under `ENABLE_NEO_MODE`: inspecting a Neo frame's variables
(`this` + locals) during a debug session / an unhandled-exception unwind. The Neo
frame is `byte* frameBase` + `AutoList mStack` (the reference region) +
`CompiledFrame.LocalInfos` (`StackSlotInfo{Offset, RefOffset, Size, RefCount}`) --
NOT the Legacy `StackObject[]`. The stacktrace instruction dump is already
Neo-adapted; this capability covers variable inspection.

## Purpose

`DebugService.GetThisInfo` / `GetLocalVariableInfo` read a frame's variables for
the debugger protocol and the unhandled-exception `ILRuntimeException` enrichment.
Under Neo they previously returned "not supported yet"; this capability records
that they now read the Neo frame via a per-shape dispatch mirroring the F-4
`ILTypeInstance` indexer (primitive -> Primitives by width; reference -> absolute
mStack index, sentinel -1 = null; CLR-struct -> ReadNeoValueType; IL-VT-local ->
placeholder, reconstruction deferred). Legacy `ExecuteR` is byte-identical.
