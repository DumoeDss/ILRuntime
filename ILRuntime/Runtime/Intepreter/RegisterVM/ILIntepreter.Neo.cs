...
namespace ILRuntime.Runtime.Intepreter
{
    public unsafe partial class ILIntepreter
    {
        ...
        // neo-float-vtreturn-opaddition: public facade for the value-type managed
        // size, so autogenerated Neo CLR bindings (which compile in the CONSUMER
        // assembly, e.g. ILRuntimeTestBase) can compute a VT param/return slot size
        // without referencing the internal Optimizer (the codegen-time
        // BindingGeneratorExtensions emits this call). Delegates to Optimizer.
        //GetNeoValueTypeManagedSize (= Unsafe.SizeOf<T> via the cached reader). The
        // autogen codegen previously emitted a direct Optimizer.GetNeoValueTypeManagedSize
        // reference, which never compiled in the consumer (Optimizer is internal) -- so
        // every regenerated VT stub was stale/broken. This wrapper is the public seam.
        public static int GetNeoValueTypeManagedSize(Type t)
        {
            return Optimizer.GetNeoValueTypeManagedSize(t);
        }
