...
| F-7B / NEO-REF-TYPE-BYREF-WRITEBACK | in-place write-back of a CALLEE-CREATED reference object through a delegate-Invoke byref (`s = s + "!"` via `ref string`) leaves the caller's slot with a DANGLING mStack index: the new object lands in the callee's frame ref region (reserved by `ExecuteNeo` at `ILIntepreter.Neo.cs:4703`), which is popped on return, so the index the byref write-back stored no longer resolves -> `Index out of range` in a later CLR string binding (`op_Inequality`). Primitive-byref write-back is unaffected (the value is written as flat bytes to the caller's frame, no mStack index). | future (route to a ref-type byref follow-up child) | F-7 same-frame fast path + byref-relativization land the byref and make primitive write-back live; ref-type write-back of a NEW object additionally needs mStack LIFETIME PROMOTION (store the new object in a caller-frame ref slot before writing the index back, or convert the frame-native byref-to-ref-slot into an mStack-slot byref the consumer writes as `mStack[idx]=obj`). The `ref string` MARSHAL+READ path already works (`NeoStep19_ByRef_String`: `ReadLength(ref s)` returns `s.Length`). | surfaced 2026-07-09 (neo-f7-delegate-byref apply); the marshal/read half is RESOLVED under F-7, the in-place-new-object write-back is the open half |
