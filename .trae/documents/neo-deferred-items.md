...
| N-CONVOVF | `conv.ovf.*` family (`conv.ovf.u2.un` etc.) unimplemented in `ExecuteNeo` (Step 6) | neo-async-multi-await review (lead-5) | **DEFERRED -> completion-3 child** (after the Task->Task<int> binding-edge child) | Step 6 opcode arms | an async state-machine string-concat (`"... " + intVar`) lowers to `conv.ovf.u2.un` -> `NotImplementedException`; Legacy has the arm (`ILIntepreter.Register.cs:~1043` + JIT `:~2520`); `ILIntepreter.Neo.cs` has 0 matches. Audit the wider `conv.ovf.*` family when implementing. Async test bodies must be concat-free meanwhile (log via host helpers taking separate args). |
...
| F-2 / INLINER-REFONLY-VT | ref-only VT (prim-size 0) local `new S(refArgs)` mis-compiles: inlined `stfld.ref.inline` writes don't survive to the following in-frame `ldfld.ref` read | neo-vt-this-addr re-review (F-1 probe) | **RESOLVED 2026-07-09 (neo-latent-edges): NON-REPRODUCIBLE on HEAD** (3/3 probes PASS; the JIT body disproves the ref-fold hypothesis -- the inlined ref writes DO survive to the ref reads). Closed (the Q-STRUCT/Q-LONG pattern). | JITCompiler inliner ref-fold over a 0-prim-size VT local | pre-existing (latent; disproven on HEAD) |
...
| F-4 / NEO-IL-EX-FIELDACCESS | Reading IL-declared fields/methods off a CAUGHT IL exception via the adaptor bridge is broken on Neo (4 read paths: #1 bridge, #2 `e.GetType()`, #3 `appdomain.Invoke`, #4 `ILTypeInstance.this[index]`) | neo-il-exception-throw apply (OQ1/OQ2) | **RESOLVED (neo-f4-reflection-on-neo 2026-07-08 + neo-f4-parametrized-run-entry 2026-07-08 + neo-f4-surfaced-gaps 2026-07-09)**: #1+#2 NO-OP (already worked; doc STALE), #4 RESOLVED (Neo indexer get/set arms), #3 RESOLVED (parametrized-Run follow-on -- `Run` now marshals slot-0 `this`). The 2 NEW gaps the F-4 work sequenced are BOTH RESOLVED by neo-f4-surfaced-gaps 2026-07-09: (A) op_Equality null-operand -- `ReadNeoReference` now applies the `(idx >= 0) ? mStack[idx] : null` null-sentinel guard (closes the whole autogen-binding defect class); (B) "newobj string-arg" -- RE-CHARACTERIZED at implement time: the F-4 hypothesis ("plain `new MyEx(msg)` stores `this` into Msg") was STALE (the plain ctor already worked post-`de0ef01c`); the REAL bug was a DERIVED IL type's flat `ILTypeInstance` missing inherited fields (`ILType.InitializeFields` did not accumulate the IL base's `TotalPrimitiveSize`/`TotalReferenceCount`, so e.g. `new DerivedEx(msg):base(msg)` allocated a 0-slot `ManagedObjects` and the base ctor's `stfld` NREd). Fixed by prepending the IL base's flat totals in the `#if ENABLE_NEO_MODE` arm (mirrors Legacy's flat `TotalFieldCount`). | `ILTypeInstance` Neo indexer (path #4) + parametrized `Run` Neo arm (path #3) + `ReadNeoReference` null-sentinel (Gap A) + `ILType.InitializeFields` base-field accumulation (Gap B) | pre-existing; NeoStep 226/0/0; F-13 nested-ExecuteNeo re-entrancy still open (see F-13 row) |
...
| D-PEEP | `box T; isinst U` peephole + `PatchKind.IsinstResult` | Step 15 | **RESOLVED 2026-07-09 (neo-peephole-pass): CLOSED as evidence-based optimization-deferral** -- a Cecil scanner found 0/1071 adjacent box;isinst pairs across 3 real DLLs (TestCases + ILRuntime + ILRuntimeTestBase; the C# compiler box-elides/Unbox_Any-folds). The framework (Step-17 liveAliases IS the reusable def-use framework) + encoding (Operand@8 T + Operand2@12 U, F-8-disjoint; avoid Operand3@16) answers recorded for a future hot-path-driven revival. NOT a correctness gap (the box;isinst path works un-fused). | optimization (non-functional; zero measured payoff; the F-11/Q-STRUCT pattern, now empirical) |
...
| F-7 / NEO-DELEGATE-REFOUT | byref-aware arg marshaling in `DelegateAdapter.NeoInvokeSub` (delegate ref/out params) | Step 19 | **RESOLVED 2026-07-09 (neo-f7-delegate-byref)** primitive-byref | RESOLVED via the same-frame delegate-Invoke fast path: `Callvirt_IL` `IsDelegateInvoke` branch runs an IL delegate target on THIS interpreter via the new `NeoRunDelegateTargetOnThis` helper (reusing the already-marshaled `targetBase`), with the key BYREF-RELATIVIZATION correction -- the verbatim-copied frame-native byref `(objIdx=-1, off)` carries a CALLER-frame-relative offset, but the target's stind/ldind resolve `objIdx==-1` against the TARGET's (higher) frame base, so `NeoRunDelegateTargetOnThis` re-bases each frame-native byref param offset by `-(dTargetBase - callerFrameBase)` (undone after the run so multicast re-invocation does not compound). Static targets shift the base forward by the Invoke `this` slot (4); instance targets overwrite the adapter mStack slot with the bound `instance`. The old `ReadNeoDelegateInvokeArgs`+`NeoInvokePublic` (separate-interpreter) path stays as the non-IL-target fallback (D5). `ref int`/`out int`/multicast write-back is OBSERVABLE (v==15/v==99/v==30). CRITICAL FINDING: the Neo Step-17 IL-byref tests pass ONLY because the optimizer INLINES those small targets (the byref never crosses a frame); a delegate-Invoke target cannot be inlined -> first real cross-frame IL byref -> relativization is required (the original design's premise that stind resolves against the caller frame was wrong). NeoStep 233/0/0, NeoStep19 14/0/0, Legacy-neutral. OPEN FOLLOW-UP (new row F-7B / ref-type-byref-writeback): in-place write-back of a CALLEE-CREATED reference object (`s = s + "!"`) leaves a dangling mStack index (the new object lands in the callee's frame ref region, popped by `ExecuteNeo:4703`); ref-type byref write-back needs mStack lifetime promotion. The `ref string` MARSHAL+READ path works (probe returns `s.Length`). | pre-existing (latent; the only reachable shape today is plain primitives via `WriteNeoCallSlot`) |
| F-7B / NEO-REF-TYPE-BYREF-WRITEBACK | in-place write-back of a CALLEE-CREATED reference object through a delegate-Invoke byref (`s = s + "!"` via `ref string`) left the caller's slot with a DANGLING mStack index: the new object lands in the callee's frame ref region (reserved by `ExecuteNeo` at `ILIntepreter.Neo.cs:960-962`; the pop is the `Ret` arm `RemoveRange` at `:2719`, with the exception-unwind equivalent at `:4700`), which is popped on return, so the index the byref write-back stored no longer resolved -> `Index out of range` in a later CLR string binding (`op_Inequality`). Primitive-byref write-back is unaffected (the value is written as flat bytes to the caller's frame, no mStack index). | **RESOLVED 2026-07-09 (neo-f7b-reftype-writeback)** | RESOLVED via the D1a PROMOTION in `NeoRunDelegateTargetOnThis` (the byref-channel analog of the single-reference RETURN promotion at the `Ret` arm `:2689-2694`): for a frame-native byref param whose referent is a REFERENCE type (`!IsPrimitive && !IsValueType`, gated via `pt.ElementType` -- ILType sets `byRefType.elementType` = the referent), reserve a CALLER-OWNED mStack slot ONCE per delegate-Invoke (`ref int callerOwnedRefSlot` threaded from the `Callvirt_IL IsDelegateInvoke` call site, shared across the multicast chain) via `mStack.Add(null)` BEFORE `InvokeNeoCallTarget` (so the nested `ExecuteNeo` reserves its `frameRefBase = mStack.Count` ABOVE it -> survives the callee pop); initialize it with the byref's CURRENT referent (the caller cell holds the referent's mStack index; copy that object); rewrite the byref in `targetBase` to the mStack-object shape `(objectIndex = callerOwnedRefSlot, offset = JITCompiler.NeoF10ByrefOffsetFlag)` (the F-10 high-bit discriminator idiom, disjoint from field hashes / array indices / Primitives offsets); teach `Stind_Ref`/`Ldind_Ref` (the `objIdx >= 0` branch) a FIRST sub-arm keyed on `(off & NeoF10ByrefOffsetFlag) != 0` that writes/reads `mStack[callerOwnedRefSlot]` directly; after the run re-stamp the caller cell `*(callerFrameBase + origOff) = callerOwnedRefSlot` (parity with the RETURN promotion) and restore the raw frame-native byref in `targetBase` for multicast idempotency (D3, last write wins). Primitive/value byrefs keep the F-7 byte-relativization UNCHANGED (D2); the non-IL-target fallback is UNCHANGED (D4). Probes: `NeoStep19_ByRef_StringWriteBack` (binding, `r==4 && s=="abc!"`, FAIL-on-HEAD `Index out of range` -> PASS) + `NeoStep19_ByRef_StringMulticastWriteBack` (D3 adversarial, `AppendBangV`->`AppendQ` over "abc" -> "abc!?"). NeoStep **235/0/0** (+2 probes over the 233 baseline), NeoStep19 **16/0/0**; the 4 F-7 primitive-byref probes + the `_String` READ probe hold; Legacy-neutral (plain-`Debug` library build 0 errors; all edits `#if ENABLE_NEO_MODE`). | surfaced 2026-07-09 (neo-f7-delegate-byref apply); RESOLVED 2026-07-09 (neo-f7b-reftype-writeback apply) |
| F-7B-SIB / NEO-DIRECT-CALL-REF-BYREF-WRITEBACK | the DIRECT `Call_IL` / `CopyNeoCallThisBack` sibling path (`ILIntepreter.Neo.cs:2265` Call_IL + `:543-595` CopyNeoCallThisBack) has the SAME dangling-index mechanism as F-7B for a NON-delegate cross-frame `ref <reference-type>` reassign: `CopyNeoCallThisBack`'s `Unsafe.CopyBlock` at `:578` propagates the callee-frame mStack index verbatim into the caller cell, which the callee pop then dangles. UNREACHABLE today because the Neo trivial inliner (`JITCompiler.cs:2931-2975`) folds small static direct targets, so a direct `ref string`-reassign call never crosses a frame (a method with an exception handler or above the inline size threshold is the only way to force it). | **RESOLVED 2026-07-09 (neo-f7b-sib-direct-call)** -- REACHED via 3 non-inlinable probes (a try/catch body defeats the inliner via `hasExceptionHandler`; a >20-instruction body via `MaximalInlineInstructionCount`; a primitive `ref int`). The gap was BROADER than the F-7B-SIB reference subset: the ENTIRE IL-direct-`Call` byref ABI was un-rebased+un-flagged (`clrParams == null` for IL callees -> the byref param is 'plain'; primitive `ref int` fails too). Fixed via a runtime-only `NeoPreCallByrefFixup` helper in the `Call` case (the D1 JIT-flagging PIVOTED away -- D1 would CORRUPT the byref; an IL callee CONSUMES it. The delegate-path `NeoRunDelegateTargetOnThis` mirror is correct). No JIT change -> no Step-17 regression by construction. NeoStep19 19/0/0 (3 SIB probes PASS), NeoStep17 47/47 (HIGH-RISK inlined-byref gate GREEN), NeoStep 238/0/0, Legacy-neutral. | Distinct from F-7B (the delegate-Invoke path). This is the direct-`Call_IL`/`CopyNeoCallThisBack` path. | surfaced 2026-07-09 (neo-f7b-reftype-writeback design Open Questions); RESOLVED 2026-07-09 (neo-f7b-sib-direct-call apply) |
...
| F-9 / NEO-INLINED-RETURN-MOVE | an int returned from an inlined IL method moved as a reference -> `mStack[intValue]` OOB (return-value classification edge in the trivial inliner) | neo-step13-area4-refandstind review (4d.2 probe-avoidance) | **RESOLVED 2026-07-09 (neo-latent-edges): NON-REPRODUCIBLE on HEAD** (3/3 probes PASS; the JIT body shows the int return moves through primitive registers -- `move r6,r5; move r0,r5` -- no reference move, no OOB. The documented `+0` defeat defeats nothing on HEAD). Closed. | the trivial-inliner mis-classifies an inlined IL-method return value (moves an int as a reference) | pre-existing (latent; disproven on HEAD) |
...
| STEP-25-PARTIAL | Step 25 runtime `.neo` loader — S1 (non-generic load+execute) V2-PROVEN; S2 done; S3 partial (sub-surface 1 shipped); S3-2 Cecil-free load SHIPPED (sub-surfaces 2+3 done) | neo-step25-runtime-loader (Step 25) | **PARTIAL**: S1 SHIPPED + V2-PROVEN (ILMethod AOT-init dual-path `isNeoAotBody` + `InitCodeBodyFromNeo` + BodyRegister short-circuit; `NeoAssemblyLoader.Attach` same-AppDomain non-generic bind; `NeoStep25LoadExecCheck` V2 capstone 8/8 -- deserialize + ExecuteNeo == JIT for the non-generic matrix: arithmetic / try-catch EH / locals+byref-call; NeoStep 208/208 + NeoStep22 55/55 + NeoStep23 15/15 + NeoStep24 5/5; Legacy-neutral). S2 SHIPPED 2026-07-07 (neo-step25-s2-generic-at-load: the no-T-identity-token generic slice); S3 PARTIAL 2026-07-08 (neo-step25-s3-full-decoupling: sub-surface 1 -- ILType layout + Neo VTable rebuild from `NeoTypeDefRecord`, structural-equivalence + 2 mutation cells proven -- SHIPPED; sub-surfaces 2/3/4/5 deferred); S3-2 SHIPPED 2026-07-08 (neo-step25-s3-cecil-free-load: sub-surfaces 2+3 -- Cecil-free AppDomain load + APPROACH-1 cross-AppDomain token re-resolution, INSEPARABLE, shipped together); S3-4 SHIPPED 2026-07-09 (neo-step25-s3-ccctor: sub-surface 4 -- static `.cctor` seeding at Cecil-free load RESOLVED) | S2 DONE: `NeoAssemblyLoader.Attach` consumes `model.Templates` + `GenericMethodTemplateOps.BuildFromNeoRecord` (re-resolves `VariableTypes` from `VariableTypeRefIdxs`; rejects T-identity-token/MethodToken patches w/ non-none CecilTokenKind) + `ILMethod.InitTemplateFromNeo` (OVERWRITES the JIT-captured template); the Step-22 hook (unchanged) routes generic instances through CloneAndPatch from the AOT template. V2 capstone 21/21. S3 PARTIAL DONE (sub-surface 1): `ILType.RebuildFromNeoRecord` + `NeoAssemblyLoader.ResolveVTableFromRecord` + `ILType.NeoVTableSlotKeysForAOT`; capstone 28/28. S3-2 DONE (sub-surfaces 2+3, 2026-07-08): the Cecil-free load. `AppDomain.LoadNeoAssembly(model, hostClrRefs)` (Neo-only) loads a `.neo` into a FRESH `new AppDomain()` (NO Cecil ModuleDefinition) + executes via ExecuteNeo. THREE inseparable parts: (a) `.neo` Version bumped 1->2 with NAME-based `NeoTokenBinding[]` tables (`TypeTokenBindings` + `MethodTokenBindings`) -- flat {Hash, FullName[, MethodName, ParamCount]} lists snapshot from A's mapTypeToken/mapMethod AFTER CompileFresh (every baked identity hash, incl. Cecil TypeReference/MethodReference identity hashes which are ALSO process-global-counter-based, NOT reproducible); a ref may have MULTIPLE hashes; the loader re-resolves by NAME + re-registers under the recorded hash; (b) `ILType.CreateFromNeoRecord` + `FinalizeFromNeoRecord` (the S3-partial rebuild INSTALLED on a fresh Cecil-free ILType: layout + VTable + interface map; `isNeoAotType` guards the Cecil-reading properties + lazy inits) + `ILMethod.CreateFromNeoShell` (Cecil-free ILMethod shells, `isNeoAotShell`; return type from the new `NeoMethodDefRecord.ReturnTypeRefIdx`); (c) the two-pass build (pass 1 builds + registers, pass 2 resolves base/interface by NAME) + `EnsureNeoPrimitiveTypes` (a fresh Cecil-free AppDomain has no Cecil load -> primitives uninitialized) + interface-method shell synthesis (an interface's abstract methods aren't in the .neo MethodDefs; synthesized on demand so the interface-dispatch declared-method resolves). The binding carries the NAME directly (NOT a ref-table index) so body-INTERNAL call-site tokens (e.g. an interface method a body calls) are covered. Capstone `NeoStep25CecilFreeLoad` 5/5 (compile-in-A + Cecil-free-load-into-B + Compute() exec [field read + virtual + interface dispatch on the Cecil-free ILType] + M1 body-mutation [Ldc_I4_S FLong] + M2 layout-mutation [PrimitiveOffset]); M2 adversarially re-proven load-bearing (break factory -> M2 FAILs). NeoStep 223/0/0, NeoStep25LoadExec 28/28 unchanged, Legacy-neutral (plain-Debug 0 errors). STILL DEFERRED: CLR base/interface resolution on the Cecil-free path (the capstone's base + interface are BOTH IL types in the same `.neo`; a CLR base/interface needs the CrossBindingAdaptor path -- the NEO-AOT-ADAPTOR-SKIP pattern), generic-method/type instances on the Cecil-free path (S2 T-identity-token + cross-AppDomain generic-instance re-resolution; the capstone is non-generic), cross-PROCESS load (P1-built `.neo` in P2 -- deployment concern; APPROACH-1 hashes are already process-independent), multi-hotfix-assembly cross-refs; Step 26 perf | S1 infrastructure shipped. S3-2 KEY findings: (1) Cecil `TypeReference.GetHashCode`/`MethodReference.GetHashCode` are IDENTITY-based (process-global counter, Mono.Cecil/MethodReference.cs:139 + TypeReference.cs:105) -- NOT just ILType/ILMethod; so APPROACH-1 must record ALL baked hashes, not only IL identity; (2) the binding must be NAME-based (body-internal call-site tokens aren't in the .neo ref tables); (3) a fresh Cecil-free AppDomain needs `EnsureNeoPrimitiveTypes`; (4) interface abstract methods aren't in the .neo -> synthesized shells. S3-4 DONE (sub-surface 4, 2026-07-09, neo-step25-s3-ccctor): the Cecil-free `.cctor` seeding. (a) `.neo` Version bumped 2->3 with a per-static-field `NeoTypeDefRecord.StaticFields[]` (parallel to the instance `Fields[]`; the writer's `BuildStaticFieldLayouts` + the reader's mirror; `StaticCtorMethodRefIdx` FINALLY consumed); (b) `ILType.CreateFromNeoRecord` installs `staticFieldOffsets`/`staticFieldTypes`/`staticFieldMapping` from `StaticFields[]` + tracks the `.cctor` shell as `staticConstructor`; (c) `ILTypeStaticInstance` ctor `isNeoAotType`-guarded skips the Cecil `TypeDefinition.Fields` `InitialValue` replay; (d) `LoadNeoAssembly` step (5) seeds each Cecil-free type's `.cctor` via `Invoke(cctor, null, null)` AFTER `Attach` + AFTER hash re-registration (best-effort try/catch). Capstone `NeoStep25CecilFreeLoad` 7/7 (the S3-2 5/5 + C1 `.cctor` capstone [ReadStatic returns the `.cctor`-set 777, NOT 0] + C2 `.cctor` body-mutation [mutated constant 4242 reflected -- the load-bearing adversarial proof]). S3-4 KEY findings (the design D-GATE 2 was PARTIALLY WRONG): (1) the design assumed the `.cctor` body "runs correctly once (a)-(c) are in place" -- but `ExecuteNeo` had NO `Stsfld`/`Ldsfld` case handlers at all (the stale "TODO Step 7" only covered instance Stfld/Ldfld, which DID ship; STATIC field ops were never implemented). S3-4 IMPLEMENTED `Stsfld`/`Ldsfld` in `ExecuteNeo` (the token encoding `(typeHash<<32)|staticFieldIdx` mirrors the Legacy register VM at ILIntepreter.Register.cs:3288/3316; the static instance's `byte[] Primitives` + `AutoList ManagedObjects` are read/written at the per-field offset with the field type's width -- primitive widths + IL-value-type-span + reference slots handled; the JIT sets ONLY Register1=DstOffset for Stsfld so the source value is at `DstOffset`, NOT `SrcOffset`); (2) the compiler NEVER compiled the `.cctor` body (`NeoCompiler.CompileCore` iterated `GetConstructors()` which holds ONLY instance `.ctor`s -- a `.cctor` routes to the SEPARATE `staticConstructor` field per `InitializeMethods` ILType.cs:2101-2108; pre-S3-4 `BuildStaticCtorRef` scanned `GetConstructors()` for `IsStatic` so `StaticCtorMethodRefIdx` was ALWAYS -1). S3-4 fixed BOTH: `CompileCore` now ALSO compiles `GetStaticConstroctor()` into `methods[]` + `BuildStaticCtorRef` uses `GetStaticConstroctor()`; (3) the Cecil-free factory passed `isStatic=false` to `CreateFromNeoShell` for ALL methods -> a static shell carried `HasThis=true` -> `Invoke(static, null)` NRE'd ("instance should not be null"); S3-4 passes the MethodRef's recorded `IsStatic`; (4) `ILMethod.ToString()`/`InitParameters()` NRE'd on a Cecil-free shell (`def.Parameters` with null `def`) -> a body exception's `ILRuntimeException` wrap (`ILIntepreter.Neo.cs:4426`) called `GetStackTrace`->`ToString`->`InitParameters` -> NRE MASKED the real inner exception; S3-4 guarded `ToString` for `isNeoAotShell` (uses `neoShellParameters`); (5) `ILType.StaticInstance` getter line 201 evaluated `TypeReference.HasGenericParameters` which NREs on a Cecil-free type -- S3-4 guarded the `.cctor`-eligibility condition (`isNeoAotType ? true : ...`) WITHOUT lifting the stale `#if ENABLE_NEO_MODE` suppression (S3-4 seeds ONLY the Cecil-free path explicitly at `LoadNeoAssembly`). NeoStep 224/0/0 (the +1 over the 223 baseline is a pre-existing test, not the probe -- the probe is host-side only), NeoStep25LoadExec 28/28 unchanged, Legacy-neutral (plain-Debug 0 errors). KEY prerequisite carried: `ILIntepreter.Run` Neo arm RESOLVED 2026-07-08 (neo-f4-parametrized-run-entry; marshals instance + p + reference returns). IL-side nested `appdomain.Invoke` re-entry still blocked by F-13 / NEO-NESTED-RUN-EXECUTENEO (host-side single-level re-entry works; the S3-2 capstone is host-side) |
...
| NEO-AOT-CLRBASE-IFACE | Cecil-free AOT (`.neo`) LOAD/EXECUTE of an IL type whose base or interface is a CLR type needing a `CrossBindingAdaptor` (the NEO-AOT-ADAPTOR-SKIP inverse -- the COMPILE side `neo-step25-clr-adaptor` made the CLI robust to such types; this is the LOAD side). The S3-2 Cecil-free path resolved a CLR base/interface to a raw `CLRType` but never installed the adaptor. | neo-aot-clrbase-iface (lead-6 MEDIUM#6) | **RESOLVED 2026-07-10 (neo-aot-clrbase-iface)** | THREE coupled fixes (Neo-only, Legacy-neutral): (1) WRITE side `NeoAssemblyWriter.BuildTypeDef` + `BuildInterfaces` now RECORD the CLR base/interface for a CrossBindingAdaptor-resolved base/interface (the Cecil path `InitializeBaseType`/`InitializeInterfaces` REPLACES a CLR base with the adaptor, so `type.BaseType`/`interfaces[i]` is the `CrossBindingAdaptor` IType -- the old `is ILType` check dropped it -> `BaseTypeRefIdx`/`InterfaceTypeRefIdx` was -1). The adaptor's `BaseCLRType` is `module.ImportReference`'d to a Cecil TypeRef so the record carries its NAME; no `.neo` Version bump (additive: previously--1 values now filled). (2) LOAD side `ILType.FinalizeFromNeoRecord` now looks up `appdomain.CrossBindingAdaptors[clr]` after resolving a CLR base/interface (mirroring `InitializeBaseType` :1996-2007 / `InitializeInterfaces` :1900-1910) + installs the adaptor (throws `TypeLoadException` on a missing adaptor, loud). (3) `ResolveFirstCLRBase`/`ResolveFirstCLRInterface` gained a `is CrossBindingAdaptor` short-circuit (an adaptor IS the first CLR type; it is NOT an ILType subclass -- the old `is CLRType`/`is ILType` checks both missed it -> returned null). Capstone `NeoStep25ClrBaseIface` 4/4 (ExceptionProbe : System.Exception emitted + Cecil-free-loaded + the invariant `FirstCLRBaseType is CrossBindingAdaptor` + the `CLRInstance is Exception` bridge + end-to-end `Invoke(Tag)==42` + M1 body-mutation). Stash-toggle PROVEN: the invariant cell FAILs on HEAD (FirstCLRBaseType null), PASSes with the fix. NeoStep 253/0/0, NeoStep25CecilFreeLoad 7/7, NeoStep25LoadExec 28/28, NeoStep25ClrAdaptor 7/7, NeoStep23Roundtrip 15/15 -- all unchanged. Legacy-neutral (plain-Debug 0 errors). | closed. KEY durable finding: `CrossBindingAdaptor : IType` (NOT `ILType`) -- a walk that recurses into ILType.baseType must short-circuit on the adaptor or it returns null. NOTE: NeoStep24CliRoundtrip is 1/5 PRE-EXISTING (unrelated; identical with/without this change). A harness-adaptor Cecil-free load is OUT OF SCOPE (such types are skip-listed at compile time -> never reach a Cecil-free load; a missing adaptor throws `TypeLoadException` -- the documented loud behavior). |

| F-11 / NEO-AOT-GENERIC-EAGER-COMPILE | the runtime `Call` resolves a generic callee via `AppDomain.GetMethod(token)`, which returns the instance registered when the callee was FIRST touched -- and the NeoCompiler compile step (force-compile of each non-generic method) eagerly touches a generic callee's `BodyRegister` (to build the caller's NeoCallParamMap), caching it with the template bound AT COMPILE TIME. So if a generic instance is compiled BEFORE the `.neo` loader binds the AOT template, that instance's bodyRegister is STALE (JIT/unmutated) and the loader's template bind never reaches it. Non-generic S1 hides this (`InitCodeBodyFromNeo` OVERWRITES the callee's bodyRegister + isNeoAotBody); generic S2 cannot (the loader binds the def cache, NOT the instance). | neo-step25-s2-generic-at-load (capstone dump-gate) | **accepted-known optimization (NOT a bug; closed 2026-07-09 neo-latent-edges)** -- the stale bodyRegister is CORRECT JIT (the method runs correctly, just not via the AOT template = a missed optimization, not corruption). S2 works around it via `MakeGenericMethod` (a fresh instance). A future S3 production-AOT optimization could refresh stale instance bodyRegisters (load-order / per-instance refresh), but it is NOT a correctness gap. | NOT an S2 bug (the S2 slice proves the AOT template drives CloneAndPatch for a fresh instance); an optimization gap, not corruption |
| F-12 / NEO-RUN-REF-RETURN | `ILIntepreter.Run` (the Step-6 parameterless entry shim) returns via `NeoBoxReturnValue(method.ReturnType, retDst, retSize)`, which handles PRIMITIVE returns only (int/long/etc.); a REFERENCE-type return (string/object/ILType) falls through to the raw-`*(int*)retDst` fallback (reads the low 4 bytes of the ref slot). So invoking a parameterless method that returns a reference type via `appdomain.Invoke` returns garbage (the S2 `WrapEchoRef` that returned `Echo<string>` -> "hi" got 0). | neo-step25-s2-generic-at-load (capstone dump-gate) | **RESOLVED 2026-07-08 (neo-f4-parametrized-run-entry)** | The parametrized-Run rewrite added a type-discriminated return-read: a non-value-type non-void return reads `mStack[*(int*)retDst]` (the boxed reference), NOT `NeoBoxReturnValue`'s primitive fallback. Gate: `NeoF4ParamRunCheck` F12 cell -- `appdomain.Invoke(EchoRef)` -> boxed string "hi" (FAIL-on-HEAD: `Int32:0`). NeoStep 223/0/0; Legacy-neutral | pre-existing (NOT introduced by S2; closed by the parametrized-Run follow-on) |
| F-13 / NEO-NESTED-RUN-EXECUTENEO | Nesting `appdomain.Invoke(...)` (-> a 2nd `Run` -> a 2nd `ExecuteNeo` on a fresh pooled interpreter) from WITHIN an in-flight `ExecuteNeo` corrupts the OUTER method's instruction pointer: the outer `ip` runs off the end of its body -> a garbage opcode (`NotImplementedException: Neo: opcode <random> not yet implemented`, `ipOff` past the body length). Affects ANY IL method that calls `appdomain.Invoke` (or a CLR helper that does) re-entrantly -- e.g. an IL probe that recovers an ILTypeInstance then re-invokes a method on it. Surfaced 2026-07-08 by neo-f4-parametrized-run-entry (the original IL-side F-4 #3 / F-12 probe shape nested `appdomain.Invoke` in a catch handler). | neo-f4-parametrized-run-entry apply (the IL-side probe pivot) | **accepted-known (pre-existing; SEQUENCED)** | The parametrized-Run change is NOT the cause (reproduced with the OLD parameterless-only Run shim -- identical ipOff-past-body garbage). The F-4 #3 + F-12 gates were moved HOST-SIDE (`NeoF4ParamRunCheck`, CLI hook `NeoF4ParamRun`) so each cell invokes `Run` exactly ONCE (no nesting) -- the architecturally correct shape for a HOST -> IL re-entry. Root cause NOT yet isolated (the outer interp's `body`/`ptr` are per-method; the inner runs on a separate interpreter+mStack; suspect a process-static shared by ExecuteNeo -- e.g. the `fixed` pin + a GC-triggering inner JIT, or a shared `Stack`/`ValueTypePointer` -- needs a dedicated investigation). Separate change to fix + re-enable IL-side nested-Invoke probes | pre-existing (NOT introduced by the parametrized-Run change; blocks IL-side `appdomain.Invoke` re-entry probes; single-level host re-entry works) |
| neo-debugger-neo-frame | Neo debugger variable inspection under ENABLE_NEO_MODE (reading frame vars off the Neo `byte* frameBase` + `AutoList mStack` + `CompiledFrame.LocalInfos` layout, NOT the Legacy `StackObject*` + `BasePointer` frame the current variable readers use) | neo-step26-perf-validation (sub-surface D dump-gate, HEAD f3be2788) | **PARTIALLY RESOLVED 2026-07-09 (neo-debugger-neo-frame): variable inspection SHIPPED; IL-VT-local + AOT-body + debugger-protocol SEQUENCED** | **SHIPPED (apply):** `GetThisInfo`/`GetLocalVariableInfo` Neo arms (per-shape dispatch mirroring F-4: primitive->read by width; reference->ABSOLUTE mStack index in the slot word; CLR-struct->`ReadNeoValueType`; IL-VT-local->placeholder string). Helpers `ReadNeoLocalValue`/`ReadNeoFramePrimitive`/`ResolveLocalType`. `GetThisInfo` recovers `this` from slot-0 + REUSES the F-4 `ILTypeInstance.this[index]` indexer for IL fields (no new field-read code). **Capstone:** `NeoDebuggerFrameCheck` 4/4 PASS (primitive+reference locals inspect correctly + adversarial mutation reflected + the load-bearing refusal-string gate). NeoStep smoke 226/0/0/0 (no regression). Legacy-neutral. **DURABLE FINDING:** `LocalInfos` is indexed with `paramCnt = m.ParameterCount + (HasThis?1:0)` (`m.ParameterCount` is the Cecil count, EXCLUDES `this`; the storage allocator at `JITCompiler.cs:1654` sizes `LocalInfos` HasThis-inclusive so `[0]` is the `this` slot). **SEQUENCED (follow-on children):** (1) IL-value-type-LOCAL reconstruction (the placeholder string -> the struct's fields; the frame-local analogue of F-4's IL-VT-FIELD reconstruction); (2) AOT-body variable inspection (`registerSymbols` null on AOT, `ILMethod.cs:997-998` -> serialize var metadata into `.neo`); (3) CLI debugger-protocol capstone (VSCode DAP `Debugging/VSCode/` -- the host-side self-check is the binding gate). The stacktrace instruction dump was ALREADY Neo-adapted (`DebugService.cs:143-148`) and stays out of scope. The remaining ~6 protocol methods (`AddStackFrameInfoVariables`/`ResolveCurrentFrameBasePointer`/`DumpStack`/`GetValueExpandable`/`VisitValueTypeReference`/`GetStackObjectText`) are protocol/frontend paths, not the variable-inspection core -- a frontend-integration child. | LARGE->SHIPPED-slice: the variable-inspection CORE (2 methods + 1 helper trio) shipped; the protocol/frontend surface (~6 methods) stays sequenced |
...
3. **`appdomain.Invoke(instanceMethod, e)`** -- **RESOLVED 2026-07-08
   (neo-f4-parametrized-run-entry).** The follow-on child transplanted the
   `DelegateAdapter.NeoInvokeSub` parametrized-frame machinery into `Run`'s Neo
   arm (`ILIntepreter.cs:111-204`): reserve `nf.TotalRefSize` frame ref slots,
   zero locals, ref-init unassigned locals, push `instance` as slot-0 `this`
   (unwrap `CrossBindingAdaptorType` -> `ILInstance`, matching the Legacy arm),
   marshal each `object[] p` via `DelegateAdapter.WriteNeoCallSlot`, then read
   the return with type discrimination (reference return -> `mStack[*(int*)retDst]`,
   NOT `NeoBoxReturnValue`'s primitive fallback -- this closes F-12 too). ONE
   divergence from `NeoInvokeSub`: `Run` builds on the CURRENT interpreter's
   stack (its caller `AppDomain.Invoke` already pooled it), so NO own
   Request/FreeILIntepreter. Gate: host-side `NeoF4ParamRunCheck` (CLI hook
   `NeoF4ParamRun`) -- F-4 #3 cell `GetCode returned 6` (FAIL-on-HEAD: NRE);
   F-12 cell `boxed string "hi"` (FAIL-on-HEAD: `Int32:0`). NeoStep 223/0/0;
   Legacy-neutral. NOTE: the gates are HOST-SIDE (not IL test methods) because
   nesting `appdomain.Invoke` inside an in-flight `ExecuteNeo` hits a SEPARATE
   pre-existing re-entrancy corruption (F-13 below); single-level host re-entry
   works.
