...
| F-7B-SIB / NEO-DIRECT-CALL-REF-BYREF-WRITEBACK | the DIRECT `Call_IL` / `CopyNeoCallThisBack` sibling path (`ILIntepreter.Neo.cs:2265` Call_IL + `:543-595` CopyNeoCallThisBack) has the SAME dangling-index mechanism as F-7B for a NON-delegate cross-frame `ref <reference-type>` reassign: `CopyNeoCallThisBack`'s `Unsafe.CopyBlock` at `:578` propagates the callee-frame mStack index verbatim into the caller cell, which the callee pop then dangles. UNREACHABLE today because the Neo trivial inliner (`JITCompiler.cs:2931-2975`) folds small static direct targets, so a direct `ref string`-reassign call never crosses a frame (a method with an exception handler or above the inline size threshold is the only way to force it). | future (sequencing note -- probe + fix behind a non-inlinable direct-call probe) | the analogous promotion in `CopyNeoCallThisBack` (copy the OBJECT into the caller's ref slot, not the index) once a non-inlinable direct-call probe surfaces it. Distinct from F-7B (which is the delegate-Invoke path, ALREADY shipped) -- this is the direct-call path. NOT blocking. | surfaced 2026-07-09 (neo-f7b-reftype-writeback design Open Questions); recorded, NOT shipped |
