...
> 当前已完成: Steps 1-10、11(接口分派)、12(帧内值类型)、12b(Move_Vt)、13(Box/Unbox areas 1-2)、14(异常处理)、15(isinst/castclass)、16(数组 rank-1)、17(ref/out + Ref Slot + ldelema)、18(CLR newobj; IL-VT newobj 推迟)、13b(CLRMethod 统一参数布局)、19(委托 ldftn/ldvirtftn/delegate newobj/NeoInvokeSub/多播)、20(异步/await sync 切片 — 基础设施 + sync Task<int>/void/exception 全绿)、+ [OPT-HARDEN](K1 FCP 修复)、[OPT-HARDEN-2](CLR-VT local 平铺字节声明)、[OPT-HARDEN-3](double-combine Operand3 union 修复)、[CATCH-COMPLETE](CheckExceptionType IL 分支)、[VT-THIS-ADDR](IL 值类型 newobj)、neo-il-exception-throw(端到端 IL 异常抛出/捕获)、neo-step13-area4 + neo-step13-area4-refandstind(CLR 绑定直调 + 值类型 this + CLR ref/out + CLR stind/ldind — D-13B Area4 全完成)、neo-step17-completion(constrained.-on-VT 全分派 + CLR 基元数组 ldelema)、neo-step17-stobj-refloop(Stobj/Ldobj ref-region 复制 + IL-VT-with-ref-fields constrained)、neo-vt-ldflda-inline(ldflda on in-frame VT)、neo-array-completion(rank-1 数组补全)、neo-k2fam-bridge(TEST-ONLY 回归守卫)、neo-opportunistic-cleanup(cgt-un 注释 + TC2 收紧)、neo-double-combine-quirk(F-8)、neo-clrstruct-field-of-il(F-10 — IL 实例 CLR-struct 字段 ldflda 偏移)。NeoStep 冒烟 190/190、NeoOptHardening 24/24、NeoStep20 9/9（HEAD 0aafdb34, 2026-07-06，15 子 portfolio 完成）。剩余: AOT 工具链 Steps 22-26、neo-step20-async-suspend、neo-array-multidim、neo-step17-generic-byref-etc、neo-peephole-isinst。各步推迟项及"落到哪个后续步骤"的映射见配套文档 [`neo-deferred-items.md`](neo-deferred-items.md)；portfolio 状态见 `openspec/changes/neo-completion-portfolio/portfolio-run.json`。
